  ...
  return (
    <div className="min-h-screen bg-[#080808] text-[#f5f0e8] p-8 lg:p-16">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-4xl font-serif text-[#c9a84c] mb-2">Inbox</h1>
        ...
        {messages.length === 0 ? (
          ...
          <div className="space-y-6">
            {messages.map((msg: { id: number; name: string; email: string; phone: string | null; budget: string; details: string; createdAt: Date }) => (
              <div key={msg.id} className="bg-[#111111]/80 backdrop-blur-md border border-[rgba(255,255,255,0.07)] rounded-2xl p-6 lg:p-8 flex flex-col md:flex-row gap-8 hover:border-[#c9a84c]/30 transition-colors">
                
                {/* Meta Info */}
                <div className="md:w-1/3 shrink-0 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center gap-3 mb-4">
                      <div className="w-10 h-10 rounded-full bg-[#c9a84c]/20 text-[#c9a84c] flex items-center justify-center font-bold text-sm">
                        {msg.name.slice(0, 2).toUpperCase()}
                      </div>
                      <div>
                        <h3 className="font-semibold text-[#f5f0e8]">{msg.name}</h3>
                        <a href={`mailto:${msg.email}`} className="text-[#c9a84c] text-xs hover:underline">{msg.email}</a>
                      </div>
                    </div>
                    
                    <div className="text-xs text-[#888888] space-y-1">
                      {msg.phone && <p>Phone: <a href={`tel:${msg.phone}`} className="text-[#c9a84c] hover:underline font-mono">{msg.phone}</a></p>}
                  ...
                  <div className="mt-6 flex gap-3">
                    <a 
                      ...
                      className="px-4 py-2 border border-[#c9a84c] text-[#c9a84c] hover:bg-[#c9a84c]/10 rounded-lg text-xs font-semibold tracking-widest uppercase transition-colors flex items-center justify-center gap-2 flex-1"
                    >
                ...
                {/* Details */}
                <div className="md:w-2/3 bg-[#080808] rounded-xl p-5 border border-[rgba(255,255,255,0.03)] selection:bg-[#c9a84c]/30">
